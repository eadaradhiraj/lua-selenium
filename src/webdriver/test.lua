...
-- Headless Chrome can open a viewport taller than the fixture, so a wheel
-- action is a no-op. Size the pad from innerHeight and scroll at a point
-- inside the viewport (element origin is out of bounds in Firefox).
function M.wheel_scroll_y(driver, dy)
    dy = dy or 600
    driver:execute_script([[
        var el = document.getElementById('scroll-pad');
        var h = Math.max((window.innerHeight || 800) * 4, 8000);
        if (el) {
    ...
    driver:scroll(0, dy, { origin = "viewport", x = 50, y = 50, duration = 250 })
