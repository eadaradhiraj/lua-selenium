...
* **CI / editors**: GitHub Actions tests Lua 5.3, 5.4, and 5.5. Lint and test jobs use Node 24 actions (`actions/checkout@v6`, `leafo/gh-actions-lua@v13`, `leafo/gh-actions-luarocks@v6`). Edge plus matching `msedgedriver` are installed so `tests/api.lua` runs on Edge. `.luarc.json` plus `---@class` annotations. Wikipedia examples stay out of `tests/run.lua`.
...
### Iteration 21 — WebAuthn round-trip, Firefox BiDi click, CI
**Done:** `add_credential` sends unpadded base64url PKCS#8 (Chrome/Edge require it) and the API suite asserts a real add/remove instead of skipping. Firefox `click_at` scrolls the target into view before `input.performActions`; headless Firefox gets an explicit 1920×1080 window. CI installs Edge and matching `msedgedriver`, tests Lua 5.3/5.4/5.5, and uses Node 24 action versions. Edge launch uses `EDGE_BIN` (not Chrome's binary).
